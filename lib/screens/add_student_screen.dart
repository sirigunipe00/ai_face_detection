...
class _AddStudentScreenState extends State<AddStudentScreen> {
  ...
  Future<void> _registerStudent() async {
    ...
    try {
      ...
      // Sync to Firestore. Local save above already succeeded, so a
      // Firestore failure (e.g. offline, API not enabled) doesn't block
      // registration - it's surfaced as a warning instead of a hard error.
      // A timeout guards against the SDK hanging on a slow/misconfigured
      // backend instead of failing fast.
      String? syncWarning;
      try {
        await _firestore.saveStudent(student).timeout(
          const Duration(seconds: 10),
        );
      } catch (e) {
        syncWarning = 'Saved locally, but cloud sync failed: $e';
      }

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(syncWarning ?? '$name registered successfully!'),
            backgroundColor: syncWarning != null ? Colors.orange : Colors.green,
          ),
        );
